...
# why). Do not add it to this list. The rewrites below target VENDORED_FILES
# explicitly (never a *.ts glob) so they can never touch env.ts.
FILES=(index Callout Pdf UploadLoader InterlinkingLinkInline)
VENDORED_FILES=()
for file in "${FILES[@]}"; do
  curl -fsSL "${BASE}/${file}.ts" -o "${DEST}/${file}.ts"
  echo "vendored ${file}.ts"
  VENDORED_FILES+=("${DEST}/${file}.ts")
done

# Upstream resolves './blockSpecs' through a path alias we do not use.
sed -i.bak "s|from '@/blockSpecs'|from './blockSpecs/index.js'|g" "${VENDORED_FILES[@]}"
...
sed -i.bak "s|from '@/env'|from './env.js'|g" "${VENDORED_FILES[@]}"
...
sed -i.bak -E "s|from '(\./[A-Za-z0-9_-]+)'|from '\1.js'|g" "${VENDORED_FILES[@]}"
...
sed -i.bak "s|COLLABORATION_SERVER_ORIGIN\.split(',')\[0\]\.replace(|(COLLABORATION_SERVER_ORIGIN.split(',')[0] ?? '').replace(|g" "${VENDORED_FILES[@]}"
rm -f "${DEST}"/*.bak
